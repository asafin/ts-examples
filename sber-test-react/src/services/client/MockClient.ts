import {ChangeTreeItemRequestModel, CategoryMenuItem, SubCategoryMenuItem, TreeList, Tree} from '../types'
...
export class MockClient extends BaseClient {
	...
	public readonly getCategoryList = async (search ?: string) : Promise<CategoryMenuItem []> => {
        ...
        if (search && search.length > 0) {
            list = list.filter(item => filterCategoryItems(item, search))
        }
	...
	public readonly getTrees = async (id: number, search?: string) : Promise<TreeList> => {
        ...
        let tree = treesData.find(item => item.recordId == id)

        if (search && search.length > 0) {
            tree.items = tree.items.filter(item => filterTree(item, search))            
        }
...
const filterCategoryItems = (item: CategoryMenuItem, search: string) => utils.findSubString(item.title, search) || checkSubCategoryMenuItemList(item.items, search)
...
const filterTree = (item: Tree, search: string) : boolean => utils.findSubString(item.title, search) || utils.findStringInLookupModelItemList(item.items, search)

const categoryList : CategoryMenuItem [] = [
    {
        recordId: 1,
